...
/** Auto-generate the image manifest by scanning public/images/{category}/ for
 *  .webp files.  Tags are derived from the filename: "hardware-chips.webp" →
 *  ["hardware", "chips"].  This means you can just drop new images into the
 *  category folders and they'll be picked up on the next fetch run. */
function buildImageManifest() {
  const manifest = {};
  for (const category of CATEGORY_KEYS) {
    const dir = path.join(FALLBACK_IMAGES_DIR, category);
    if (!fs.existsSync(dir)) { manifest[category] = []; continue; }
  ...
  // Write back so it can be inspected / version-controlled if desired
  const out = { _comment: "Auto-generated by fetch-news.js — do not edit manually. Drop .webp files into each category folder; tags are derived from the filename.", ...manifest };
